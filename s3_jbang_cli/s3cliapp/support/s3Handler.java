    ...
    public s3Handler(){}
        ...
        S3Client s3Client = null;
            ...
            switch (operation.operation()) {
                ...
                System.err.println("CRITICAL: Certificate or KeyStore setup failed: " + e.getMessage());
        } catch (IOException e) {
            ...
            System.err.println(String.format("CLIENT NETWORK ERROR: Could not reach or establish a secure connection to the server. Details: [%s]", e.getMessage()));
            if (e.getCause() instanceof javax.net.ssl.SSLHandshakeException) {
                System.err.println("-> The server certificate is still rejected. Double check your custom .crt/.pem file.");
            }
        } finally {
            if (s3Client != null) {
                s3Client.close();
